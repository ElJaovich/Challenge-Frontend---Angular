...
export class DishService {
  ...
  async getNormalDish(): Promise<any>{
    ...
    for (let i = 0; i < normalDish.results.length; i++) {
      ...
      normalDishData.push({title: normalDish.results[i].title, summary: infoDish.summary, image: infoDish.image, price: infoDish.pricePerServing, healthScore: infoDish.healthScore, timeReady: infoDish.readyInMinutes})
    } 
   ...
   async getVeganDish(): Promise <any> {
      ...
      for (let i = 0; i < veganDish.results.length; i++) {
        ...
        const infoDish = await this.http.get(`https://api.spoonacular.com/recipes/${idDish}/information?apiKey=${this.token}`)
        veganDishData.push({title: veganDish.results[i].title, summary: infoDish.summary, image: infoDish.image, price: infoDish.pricePerServing, healthScore: infoDish.healthScore, timeReady: infoDish.readyInMinutes})
      } 
